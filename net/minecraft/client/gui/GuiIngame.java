...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(final float partialTicks) {
        ...
        GlStateManager.enableBlend();
        if (HudList.getCrosshair().isEnabled() && this.showCrosshair()) {
            this.drawHorizontalLine(204, 10 + i / 2, 120, new Color(51, 204, 255).getRGB());
            this.drawVerticalLine(213, -83 + i / 2, 110, new Color(51, 204, 255).getRGB());
        }
        else {
        ...
        if (!(this.mc.currentScreen instanceof HUDConfigScreen)) {
            SeaClient.INSTANCE.hudList.renderMod();
        }
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        ...
        try {
            if (HudList.getScore().isEnabled()) {
                ...
                for (final Score score2 : collection) {
                    ...
                    this.getFontRenderer().drawString(s2, l1, j1 - m * this.getFontRenderer().FONT_HEIGHT, 553648127);
