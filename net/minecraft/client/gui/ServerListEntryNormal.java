...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry
{
    ...
    public void drawEntry(final int slotIndex, final int x, final int y, final int listWidth, final int slotHeight, final int mouseX, final int mouseY, final boolean isSelected) {
        ...
        final boolean isFeatureServer = this.server instanceof ServerDataFeatured;
        if (isFeatureServer) {
        ...
        Minecraft.fontRendererObj.drawString(this.server.serverName, x + 32 + 3, y + 1, 16777215);
        final List<String> list = Minecraft.fontRendererObj.listFormattedStringToWidth(this.server.serverMOTD, listWidth - 32 - 2);
        for (int i = 0; i < Math.min(list.size(), 2); ++i) {
            Minecraft.fontRendererObj.drawString(list.get(i), x + 32 + 3, y + 12 + Minecraft.fontRendererObj.FONT_HEIGHT * i, 8421504);
        }
        final String s2 = flag3 ? (EnumChatFormatting.DARK_RED + this.server.gameVersion) : this.server.populationInfo;
        final int j = Minecraft.fontRendererObj.getStringWidth(s2);
        Minecraft.fontRendererObj.drawString(s2, x + listWidth - j - 15 - 2, y + 1, 8421504);
        int k = 0;
        String s3 = null;
        int l;
        String s4;
