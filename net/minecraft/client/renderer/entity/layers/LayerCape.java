...
public class LayerCape implements LayerRenderer<AbstractClientPlayer>
{
    ...
    static {
        LayerCape.CAPEANIMATED = new ResourceLocation("textures/seaclient/cape/cape.png");
    }
    ...
    public void doRenderLayer(final AbstractClientPlayer entitylivingbaseIn, final float p_177141_2_, final float p_177141_3_, final float partialTicks, final float p_177141_5_, final float p_177141_6_, final float p_177141_7_, final float scale) {
        if (CosmeticBoolean.Cape1() && entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE) && entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().getSession().getUsername()) && CosmeticBoolean.Cape1() && entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().getSession().getUsername())) {
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            this.playerRenderer.bindTexture(LayerCape.CAPEANIMATED);
