...
public class CosmeticHalo2 extends CosmeticBase
{
    private final ModelHalo modelHalo;
    private static final ResourceLocation HALOGREEN;
    
    static {
        HALOGREEN = new ResourceLocation("textures/seaclient/halo/green.jpg");
    }
    
    public CosmeticHalo2(final RenderPlayer renderPlayer) {
        super(renderPlayer);
    ...
    public void render(final AbstractClientPlayer player, final float limbSwing, final float limbSwingAmount, final float partialTicks, final float ageInTicks, final float headYaw, final float headPitch, final float scale) {
        if (CosmeticBoolean.shouldRenderTopHat(player) && CosmeticBoolean.isHalogreen()) {
            GlStateManager.pushMatrix();
            this.playerRendner.bindTexture(CosmeticHalo2.HALOGREEN);
    ...
    private class ModelHalo extends CosmeticModelBase
    {
        ...
        public ModelHalo(final RenderPlayer player) {
            super(player);
            this.hat = CosmeticBoolean.isHalogreen();
        ...
        public void render(final Entity entityIn, final float limbSwing, final float limbSwingAmount, final float ageInTicks, final float headYaw, final float headPitch, final float scale) {
            ...
            Minecraft.getMinecraft().getTextureManager().bindTexture(CosmeticHalo2.HALOGREEN);
            GlStateManager.disableLighting();
            final ModelRenderer modelrenderer = CosmeticBase.this.bindTextureAndColor(Color.WHITE, CosmeticHalo2.HALOGREEN, this.halo, null);
