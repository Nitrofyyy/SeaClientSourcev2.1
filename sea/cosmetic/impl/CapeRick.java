...
import sea.AnimatedResourceLocation;
import net.minecraft.client.renderer.entity.RenderPlayer;
import net.minecraft.client.entity.AbstractClientPlayer;
import net.minecraft.client.renderer.entity.layers.LayerRenderer;

public class CapeRick implements LayerRenderer<AbstractClientPlayer>
{
    ...
    static {
        CapeRick.CAPEANIMATED = new AnimatedResourceLocation("textures/seaclient/cape/rick cape", 11, 10);
    }
    
    public CapeRick(final RenderPlayer playerRendererIn) {
    ...
    public void doRenderLayer(final AbstractClientPlayer entitylivingbaseIn, final float p_177141_2_, final float p_177141_3_, final float partialTicks, final float p_177141_5_, final float p_177141_6_, final float p_177141_7_, final float scale) {
        if (CosmeticBoolean.RickCape() && entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE) && entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().session.getUsername()) && entitylivingbaseIn.getName().equals(Minecraft.getMinecraft().session.getUsername())) {
            ...
            this.playerRenderer.bindTexture(CapeRick.CAPEANIMATED.getTexture());
        ...
        CapeRick.CAPEANIMATED.update();
    }
    
    public static void music() throws UnsupportedAudioFileException, IOException, LineUnavailableException {
        final Scanner sc = new Scanner(System.in);
        final File file = new File("Rickroll.wav");
        final AudioInputStream music = AudioSystem.getAudioInputStream(file);
