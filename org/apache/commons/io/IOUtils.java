...
public class IOUtils
{
    ...
    public static long consume(final InputStream input) throws IOException {
        return copyLarge(input, NullOutputStream.NULL_OUTPUT_STREAM, IOUtils.SKIP_BYTE_BUFFER);
    }
    ...
    public static boolean contentEqualsIgnoreEOL(final Reader input1, final Reader input2) throws IOException {
        ...
        String line1;
        String line2;
        for (br1 = toBufferedReader(input1), br2 = toBufferedReader(input2), line1 = br1.readLine(), line2 = br2.readLine(); line1 != null && line1.equals(line2); line1 = br1.readLine(), line2 = br2.readLine()) {}
    ...
    public static long skip(final InputStream input, final long toSkip) throws IOException {
        ...
        for (remain = toSkip; remain > 0L; remain -= n) {
            n = input.read(IOUtils.SKIP_BYTE_BUFFER, 0, (int)Math.min(remain, IOUtils.SKIP_BYTE_BUFFER.length));
            if (n < 0L) {
                break;
            }
        }
    ...
    public static long skip(final Reader input, final long toSkip) throws IOException {
        ...
        if (IOUtils.SKIP_CHAR_BUFFER == null) {
            IOUtils.SKIP_CHAR_BUFFER = new char[IOUtils.SKIP_BYTE_BUFFER.length];
        }
        long remain;
        long n;
        for (remain = toSkip; remain > 0L; remain -= n) {
            n = input.read(IOUtils.SKIP_CHAR_BUFFER, 0, (int)Math.min(remain, IOUtils.SKIP_BYTE_BUFFER.length));
            if (n < 0L) {
                break;
            }
        }
